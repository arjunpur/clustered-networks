...
input stimulus for one cluster and tracked the Fano factor over time. The
...
a dip in Fano Factor compared to the uniform network. This is consistent with the 
original paper's finding of stimulus-quenched spiking variability. 

(Figure~\ref{fig:fano_time_stimulus}).

\begin{figure}[!htbp]
    \centering
    \IfFileExists{figures/fano_factor_higher_input_stimulus_response.pdf}{%
        \includegraphics[width=\columnwidth]{fano_factor_higher_input_stimulus_response}%
    }{%
        \fbox{\parbox{0.95\columnwidth}{\centering
        Time-dependent Fano-factor figure not found.\\
        Generate figures before final compilation.}}%
    }
    \caption{Fano factor as a function of time under input stimulus. The
    clustered network exhibits larger variability changes
    over time than the uniform network.}
